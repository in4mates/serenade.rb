...
PropertyAccessor = (function() {
  ...
  PropertyAccessor.prototype.trigger = function() {
    var changes, name, names, value, _i, _len, _ref;
    ...
    for (name in changes) {
      ...
      value = changes[name];
      this.object[name + "_property"].clearCache();
      this.object[name + "_property"].event.trigger(value);
    }
    return triggerGlobal(this.object, names);
  };
...
Property = {
  ...
  binding: function(ast, node, model, controller) {
    ...
    if (typeof controller.twoWayBound === "function") {
